...
const CURSOR_MAP: Record<CursorKey, { text?: string; imageSrc: string }> = {
  ...
  search:  { imageSrc: '/icons/ninja_kirby.gif' },
  x:       { text: 'X',        imageSrc: '/images/x_sm.webp' },
...
export default function HeroSection({ onSearchOpen, onGoLHoverChange }: HeroSectionProps) {
  ...
  const activeCursorImageSrc = activeCursor === 'theme'
    ...
    : activeCursor ? CURSOR_MAP[activeCursor].imageSrc : '';
  const activeCursorText = activeCursor && activeCursor !== 'theme' && activeCursor !== 'search'
    ? CURSOR_MAP[activeCursor].text
    : undefined;
  ...
  return (
    <>
      <div id="hero-section" className="relative h-[100dvh] flex flex-col items-center justify-center bg-transparent pt-10 px-4 pb-4 md:p-8 snap-start snap-always transition-colors duration-300">
        ...
        <div className="md:hidden absolute top-4 right-4 z-10 flex rounded-full backdrop-blur-md shadow-lg overflow-hidden border border-gray-200/50 dark:border-gray-700/50">
          <button
            ...
            className="h-10 w-10 bg-white/60 hover:bg-white/70 dark:bg-[#2A2A2A]/60 dark:hover:bg-[#2A2A2A]/70 text-gray-700 dark:text-gray-200 flex items-center justify-center transition-all duration-300 cursor-none blur-on-hover"
            {...cursor('theme')}
          >
      ...
      {activeCursor && (
        <HoverCursor text={activeCursorText} imageSrc={activeCursorImageSrc} />
      )}
    </>
  );
}
